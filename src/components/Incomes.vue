      ...
      <ul>
        <li v-for="(income, index) in incomes" :key="index">
          ...
          <div>
            ...
            <button @click.prevent="$emit('removeItem', index,'incomes')" class="remove"></button>
          </div>
        </li>
      </ul>
      <button v-if="incomes.length >= 2" @click.prevent="$emit('removeList', 'incomes')" class="app__btn">Delete All</button>
    </div>
  </div>
</template>
<script>
